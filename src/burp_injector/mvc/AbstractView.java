package burp_injector.mvc;
import burp_injector.enums.ScriptType;
import burp_injector.event.EventEmitter;
import burp_injector.util.RegexValidityDocumentListener;
import javax.swing.*;
...
public abstract class AbstractView<TEvent extends Enum<TEvent>, TModel extends AbstractModel<TModelEvent>, TModelEvent extends Enum<TModelEvent>> implements PropertyChangeListener, EventEmitter<TEvent> {
    ...
    protected void attach(JTabbedPane field, TEvent event) {
        field.addChangeListener(new ChangeListener() {
            @Override
            public void stateChanged(ChangeEvent e) {
                switch ( field.getTitleAt(field.getSelectedIndex()).toUpperCase() ) {
                    case "DECODE":
                        emit(event, null, ScriptType.DECODE);
                        break;
                    case "ENCODE":
                        emit(event, null, ScriptType.ENCODE);
                        break;
                    case "PAYLOAD":
                        emit(event, null, ScriptType.PAYLOAD);
                        break;
                }
            }
        });
